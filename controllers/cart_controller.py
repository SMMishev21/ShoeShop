...
def sort_cart_items(items, criteria):
    if criteria == "price":
...
def cart():
    ...
    items = sort_cart_items(items, sort_by)
...
def checkout():
    user_id = _current_user_id()
    if not user_id:
        flash("Трябва да сте влезли, за да продължите.", "error")
        return redirect(url_for("auth.login"))
    ...
    if request.method == "POST":
        ...
        if not address or not payment or not cart_items:
            flash("Попълнете всички полета и имайте продукти в кошницата.", "error")
